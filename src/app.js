  ...
  app.use(
    helmet({
      contentSecurityPolicy: {
        directives: {
          defaultSrc: ["'self'"],
          scriptSrc: ["'self'", "'unsafe-inline'", "https://cdn.pendo.io", "https://app.pendo.io"],
          connectSrc: ["'self'", "https://app.pendo.io"],
          imgSrc: ["'self'", "data:", "https://cdn.pendo.io", "https://app.pendo.io", "https://pendo-static-5763789454311424.storage.googleapis.com"],
          styleSrc: ["'self'", "'unsafe-inline'", "https://cdn.pendo.io", "https://app.pendo.io"],
  ...
  if (config.PENDO_API_KEY) {
    swaggerOptions.customJsStr = `
      (function(apiKey){
        ...
        if(!visitorId){
          visitorId='anon-'+Math.random().toString(36).substr(2,9)+Date.now().toString(36);
          localStorage.setItem('pendo_visitor_id',visitorId);
        }
        pendo.initialize({visitor:{id:visitorId}});
